...
function login() {
  ...
  .then(data => {
    if (data.success) {
      ...
      if (data.status) {
        localStorage.setItem('csk4_mongo', data.status.mongo ? '1' : '0');
        localStorage.setItem('csk4_tg', data.status.telegram ? '1' : '0');
      }
      showDashboard();
    } else {
...
function deleteDevice(id) {
  ...
  fetch(SERVER_URL + '/api/admin/device/' + id, { method: 'DELETE', headers: authHeaders() }).then(() => loadData());
}
